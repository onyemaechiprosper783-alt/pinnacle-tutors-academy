...
const STUDENT_PREFIXES = [
  ...
  '/millionaire', '/results', '/profile', '/settings', '/class-notes',
  '/subjects', '/progress', '/leaderboard', '/community',
];
const PROTECTED_PREFIXES = [
  '/practice', '/mock', '/cbt', '/challenge', '/millionaire', '/class-notes', '/subjects',
];
...
export async function middleware(request: NextRequest) {
  ...
  if (user && PROTECTED_PREFIXES.some((p) => path.startsWith(p)) && new Date() >= ACTIVATION_ONLY_AT) {
...
export const config = {
  matcher: [
    ...
    '/profile/:path*', '/settings/:path*', '/class-notes/:path*',
    '/subjects/:path*', '/progress/:path*', '/leaderboard/:path*',
    '/community/:path*', '/admin/:path*',
  ],
};
