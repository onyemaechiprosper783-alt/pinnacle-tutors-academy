...
interface ReviewQuestion { question_id: string; selected_answer: string | null; is_correct: boolean | null; question: { question_text: string; option_a: string; option_b: string; option_c: string; option_d: string; correct_answer: string; explanation: string | null; subjects?: { name: string } } }
interface Breakdown { correct: number; total: number }
interface WeakTopic extends Breakdown { topic: string; percentage: number }
interface ResultsResponse { attempt?: ExamAttempt; questions?: ReviewQuestion[]; subject_breakdown?: Record<string, Breakdown>; topic_breakdown?: Record<string, Breakdown>; weak_topics?: WeakTopic[]; results_hidden?: boolean; message?: string }
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/exams/${params.attemptId}`, { cache: 'no-store' }).then((response) => response.json()).then((result) => { if (!cancelled) setData(result); }).catch(() => { if (!cancelled) setData({ message: 'Could not load this result.' }); }).finally(() => { if (!cancelled) setLoading(false); });
  ...
  const config = attempt.config && typeof attempt.config === 'object' ? attempt.config as { round_id?: string; results_released?: boolean; challenge?: { round_id?: string; results_released?: boolean } } : {};
  const isCbt = attempt.mode === 'cbt';
  const isUtmeChallenge = attempt.mode === 'utme_challenge' || (isCbt && !!(config.round_id ?? config.challenge?.round_id));
  const resultsReleased = Boolean(config.results_released || config.challenge?.results_released) || data?.results_hidden === false;

  if (isUtmeChallenge && data?.results_hidden === true && !resultsReleased) return <div className="mx-auto max-w-xl px-4 py-10"><div className="pta-card p-8 text-center"><div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-accent-100 text-3xl dark:bg-accent-950/50">✓</div><h1 className="text-2xl font-bold text-[var(--foreground)]">Challenge Submitted</h1><p className="mt-4 leading-relaxed text-[var(--muted)]">Your UTME Challenge has been submitted successfully.</p><div className="mt-5 rounded-xl bg-accent-50 p-4 text-sm font-semibold text-accent-800 dark:bg-accent-950/40 dark:text-accent-200">🏆 Your results are currently hidden.</div><p className="mt-4 text-sm leading-relaxed text-[var(--muted)]">The administrator will release the official challenge results. Your score and ranking will appear here after release.</p><Link href="/dashboard" className="mt-6 block rounded-xl bg-brand-700 px-5 py-3 font-semibold text-white">Back to Dashboard</Link></div></div>;
  ...
  const score = isCbt || isUtmeChallenge ? `${attempt.score ?? 0}/400` : `${attempt.score ?? 0}%`;
  const title = isUtmeChallenge ? 'UTME Challenge Result' : isCbt ? 'JAMB CBT Result' : 'Your Result';

  return <div className="mx-auto max-w-3xl px-4 py-6 sm:py-8">
    <div className="mb-6"><p className="text-xs font-black uppercase tracking-[0.2em] text-accent-600 dark:text-accent-300">Performance report</p><h1 className="mt-1 text-2xl font-black text-[var(--foreground)]">{title}</h1><p className="mt-1 capitalize text-sm text-[var(--muted)]">{isCbt ? '180-question CBT · 400-mark scale' : isUtmeChallenge ? 'UTME Challenge · 400-mark scale' : `${attempt.mode} exam`} · {minutes} min used</p></div>
    <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4"><Stat label="Score" value={score} tone="brand" /><Stat label="Correct" value={attempt.correct_count ?? 0} tone="brand" /><Stat label="Incorrect" value={attempt.incorrect_count ?? 0} tone="red" /><Stat label="Unanswered" value={attempt.unanswered_count ?? 0} tone="slate" /></div>
...
function Stat({ label, value, tone }: { label: string; value: string | number; tone: 'brand' | 'red' | 'slate' }) { const colors = { brand: 'text-brand-700 dark:text-brand-300', red: 'text-red-700 dark:text-red-300', slate: 'text-slate-700 dark:text-slate-300' }; return <div className="pta-card p-4 text-center"><p className={`text-2xl font-black ${colors[tone]}`}>{value}</p><p className="text-xs font-medium text-[var(--muted)]">{label}</p></div>; }
