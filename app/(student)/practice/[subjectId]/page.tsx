...
export default function PracticePage() {
  ...
  /*
   * Check how many questions are available whenever
   * the student's filters change.
   */
  useEffect(() => {
    let cancelled = false;
    ...
    async function checkAvailability() {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(
            data.error ??
              'Could not check available questions.'
          );
        }
        ...
        if (!cancelled) {
          ...
          /*
           * If the current number is greater than what
           * is available, automatically select the
           * largest valid option.
           */
          const validCounts = QUESTION_COUNTS.filter(
    ...
    try {
      if (
        ...
        throw new Error(
          ...
          } available for these selections.`
        );
      }
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
          ...
          <div className="mb-7">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">
              Choose your exam type, year, difficulty
            ...
            {checkingAvailability ? (
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Try changing the year, difficulty or
                  exam type.
                </p>
              </div>
            ) : (
          ...
          <div className="space-y-6">
            ...
            <section>
              ...
              <select
                ...
                className="min-h-[50px] w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-slate-800 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100 disabled:bg-slate-100 disabled:text-slate-400"
              >
                {validQuestionCounts.length > 0 ? (
                  validQuestionCounts.map(
                    (count) => (
                      <option
                        key={count}
                        value={count}
                  ...
                  <option value={5}>
                    No standard question count available
                  </option>
                )}
              </select>
