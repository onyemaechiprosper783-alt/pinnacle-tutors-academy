...
const ACTIVATION_START = new Date('2026-09-17T00:00:00+01:00');
const PRODUCT_END = new Date('2026-10-01T00:00:00+01:00');
...
export default async function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  const now = new Date();
  const showProductKey = now < PRODUCT_END;
  const showActivationKey = now >= ACTIVATION_START;
  const navGroups = [
    ...
    { title: 'Account', items: [
      { href: '/profile', label: 'Profile', icon: '👤' }, { href: '/settings', label: 'Settings', icon: '⚙️' },
      ...(showProductKey ? [{ href: '/product-key', label: 'Product Key', icon: '🔑' }] : []),
      ...(showActivationKey ? [{ href: '/activation-key', label: 'Activation Key', icon: '🔐' }] : []),
    ] },
