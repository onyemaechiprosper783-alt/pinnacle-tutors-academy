...
export async function GET() {
  ...
  const { data, error } = await supabase.from('announcements').select('*').eq('is_active', true).order('created_at', { ascending: false }).limit(10);
...
export async function POST(request: Request) {
  ...
  const { data, error } = await admin.from('announcements').insert({ ...parsed.data, created_by: caller.id }).select().single();
  if (error) return NextResponse.json({ error: 'Could not post announcement.' }, { status: 500 });

  await sendPushToStudents({ title: parsed.data.title, body: parsed.data.body, url: '/announcements' });
  return NextResponse.json(data);
}
