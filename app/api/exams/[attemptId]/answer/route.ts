...
export async function POST(request: Request, { params }: { params: Promise<{ attemptId: string }> }) {
  ...
  const { data: attempt } = await admin.from('exam_attempts').select('id, student_id, mode, status').eq('id', attemptId).single();

  if (!attempt || attempt.student_id !== caller.id) return NextResponse.json({ error: 'Attempt not found.' }, { status: 404 });
  if (attempt.status !== 'in_progress') return NextResponse.json({ error: 'This exam has already been submitted.' }, { status: 409 });

  const { error: updateError } = await admin.from('attempt_questions').update({
    selected_answer: parsed.data.selected_answer,
    answered_at: new Date().toISOString(),
    time_spent_seconds: parsed.data.time_spent_seconds ?? null,
  }).eq('attempt_id', attemptId).eq('question_id', parsed.data.question_id);

  if (updateError) return NextResponse.json({ error: 'Could not save answer.' }, { status: 500 });

  if (attempt.mode === 'practice') {
    const { data: question, error: questionError } = await admin.from('questions').select('correct_answer, explanation').eq('id', parsed.data.question_id).single();
    const correctAnswer = typeof question?.correct_answer === 'string' ? question.correct_answer.trim().toUpperCase() : '';
