...
export async function POST(request: Request, { params }: { params: Promise<{ attemptId: string }> }) {
  ...
  if (isChallenge) {
    ...
    const { error: leaderboardError } = await admin.from('leaderboard_entries').upsert({ attempt_id: attemptId, student_id: caller.id, category: roundId, score: fourHundredScore, time_used_seconds: timeUsedSeconds }, { onConflict: 'attempt_id' });
  ...
  const { error: normalLeaderboardError } = await admin.from('leaderboard_entries').upsert({ attempt_id: attemptId, student_id: caller.id, category: normalCategory, score: savedScore, time_used_seconds: timeUsedSeconds }, { onConflict: 'attempt_id' });
