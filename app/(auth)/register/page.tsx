...
export default function RegisterPage() {
  ...
  return <main className="min-h-screen bg-white px-5 py-10 text-slate-900"><div className="mx-auto flex min-h-[90vh] max-w-md flex-col justify-center"><div className="mb-8 text-center"><Link href="/" className="inline-flex flex-col items-center"><div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl bg-white shadow-md ring-1 ring-slate-100"><img src="/pinnacle-logo.png" alt="Pinnacle Tutors Academy" className="h-full w-full object-contain" /></div><h1 className="mt-4 text-2xl font-black">Pinnacle Tutors</h1><p className="mt-1 text-sm font-semibold text-orange-600">Academy</p></Link></div><section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl sm:p-8"><h2 className="text-2xl font-black">Create your account 🚀</h2><p className="mt-2 text-sm text-slate-500">Start preparing smarter for JAMB and WAEC with Pinnacle Tutors Academy.</p>{serverError && <div className="my-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{serverError}</div>}<form onSubmit={handleSubmit} noValidate><FormField label="Full name" name="full_name" value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} error={errors.full_name} /><FormField label="Email" name="email" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} error={errors.email} /><FormField label="Phone number" name="phone" type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} error={errors.phone} /><div className="mb-4"><label className="mb-1.5 block text-sm font-medium">What are you preparing for?</label><select value={form.exam_target} onChange={(e) => setForm({ ...form, exam_target: e.target.value })} className="w-full rounded-lg border border-slate-300 bg-white px-4 py-3"><option value="jamb">JAMB / UTME</option><option value="waec">WAEC</option><option value="both">Both</option></select></div><FormField label="Password" name="password" type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} error={errors.password} /><FormField label="Product/Activation Key" name="access_key" value={form.access_key} onChange={(e) => setForm({ ...form, access_key: e.target.value })} error={errors.access_key} /><div className="mt-2 flex gap-2 text-xs">{showProductKeyRequest && <button type="button" onClick={() => requestKey('Product Key')} className="font-semibold text-emerald-700">Request Product Key</button>}{showActivationKeyRequest && <button type="button" onClick={() => requestKey('Activation Key')} className="font-semibold text-emerald-700">Request Activation Key</button>}</div><Button type="submit" fullWidth loading={loading} className="mt-4">Create account</Button></form></section></div></main>;
}
