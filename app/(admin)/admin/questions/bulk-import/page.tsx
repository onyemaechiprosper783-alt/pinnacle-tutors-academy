...
export default function BulkImportPage() {
  ...
  async function handleCommit() {
    ...
    try {
      const questionsToImport: ParsedQuestion[] = [
        ...preview.valid.filter((_, i) => !excludedIndices.has(i)),
        ...preview.duplicates
      ...
      const res = await fetch('/api/bulk-import/commit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ batch_id: preview.batch_id, questions: questionsToImport }),
      });
      const data = await res.json();
  ...
  return (
    <div className="max-w-4xl">
      <h1 className="text-2xl font-bold text-slate-900">Bulk Import Questions</h1>
      <p className="mt-1 text-slate-500">
        Paste any number of questions below, preview them, then confirm the import.
        Nothing is written to the question bank until you click Import.
      </p>

      {result && (
        <div className="mt-4 rounded-lg bg-emerald-50 px-4 py-3 text-emerald-800">
          Imported {result.imported} questions{result.failed > 0 && ` (${result.failed} failed — see errors above)`}.
        </div>
      )}
      ...
      {!preview && (
        <>
          ...
          />
          <Button onClick={handlePreview} loading={loading} disabled={!rawText.trim()} className="mt-3">
            Preview import
          </Button>
        </>
      )}
      ...
      {preview && (
        <div className="mt-4">
          <div className="mb-4 grid grid-cols-3 gap-3">
            <Stat label="Detected" value={preview.total_detected} tone="slate" />
            <Stat label="Valid" value={preview.valid.length} tone="emerald" />
            <Stat label="Needs fixing" value={preview.invalid.length} tone="red" />
          </div>
              ...
              <div className="space-y-2">
                {preview.invalid.map((err, i) => (
                  <div key={i} className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm">
                    <p className="font-medium text-red-700">{err.reason}</p>
                    <pre className="mt-1 whitespace-pre-wrap text-xs text-slate-600">{err.raw_text}</pre>
                  </div>
                ))}
              </div>
              <p className="mt-2 text-sm text-slate-500">
                Fix these in your source text and re-paste — valid questions below are unaffected.
              </p>
            </section>
          )}
          ...
          {preview.duplicates.length > 0 && (
            <section className="mb-6">
              <h2 className="mb-2 font-semibold text-amber-700">
                {preview.duplicates.length} likely duplicate(s) — excluded by default
              </h2>
              <div className="space-y-2">
                {preview.duplicates.map((d, i) => (
                  <label key={i} className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm">
                    <input
                      type="checkbox"
            ...
            <section className="mb-6">
              ...
              <div className="max-h-96 space-y-2 overflow-y-auto">
                {preview.valid.map((q, i) => (
                  <label key={i} className="flex items-start gap-2 rounded-lg border border-slate-200 bg-white p-3 text-sm">
                    <input
                      type="checkbox"
                    ...
                    <span>
                      ...
                      <span className="ml-2 text-xs text-slate-400">
                        {q.subject}{q.topic ? ` · ${q.topic}` : ''} · {q.correct_answer}
                      </span>
                    </span>
                  </label>
                ))}
          ...
          <div className="flex gap-3">
            <Button onClick={handleCommit} loading={loading} disabled={preview.valid.length === excludedIndices.size && includeDuplicates.size === 0}>
...
function Stat({ label, value, tone }: { label: string; value: number; tone: 'slate' | 'emerald' | 'red' }) {
  const colors = {
    slate: 'text-slate-900', emerald: 'text-emerald-700', red: 'text-red-700',
  };
  return (
