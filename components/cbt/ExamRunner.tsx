...
interface AttemptResponse {
  attempt?: {
    config?: {
      challenge?: { global_deadline?: string | null } | null;
    } | null;
  };
}
...
export function ExamRunner({ attemptId, mode, questions, durationSeconds }: ExamRunnerProps) {
  ...
  const cbtSections = useMemo<CbtSection[]>(() => {
    ...
    Array.from(otherGroups.entries()).slice(0, 3).forEach(([id, group], index) => {
    ...
    if (normalizedQuestions.length >= 180) {
      const fallback = [
  ...
  const activeSection = mode === 'cbt'
    ? cbtSections.find((section) => section.key === activeSectionKey) ?? cbtSections[0]
    : null;
  ...
  return (
    <div className="min-h-[100svh] bg-slate-950 text-white">
      <div className="mx-auto flex min-h-[100svh] w-full max-w-6xl flex-col px-3 py-2 sm:px-5 sm:py-3">
        <header className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-800 pb-2">
          <div className="min-w-0"><div className="truncate text-xs font-bold uppercase tracking-wider text-accent-300 sm:text-sm">{mode === 'cbt' ? 'JAMB CBT' : mode === 'mock' ? 'Mock Examination' : 'Practice'}</div><div className="mt-0.5 text-[11px] text-slate-400">{answeredCount}/{questions.length} answered</div></div>
          {mode === 'cbt' && challengeLoading ? <div className="rounded-full bg-slate-800 px-3 py-1.5 text-xs text-slate-300">Checking time…</div> : <div className={`rounded-full px-3 py-1.5 text-xs font-black sm:px-4 sm:text-sm ${timerIsLow ? 'bg-red-600 text-white' : 'bg-accent-500 text-slate-950'}`}>⏱ {displayTimer}</div>}
        </header>
        ...
        <main className="min-h-0 flex-1">
          <div className="h-full rounded-2xl border border-slate-800 bg-slate-900/95 p-3 shadow-xl sm:p-5">
            <div className="mb-3 flex items-center justify-between gap-2"><span className="rounded-lg bg-slate-800 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">{activeSection?.name ?? 'Question'}</span><span className="text-[10px] text-slate-500">{answeredInCurrentSection}/{sectionQuestions.length} answered</span></div>
            <p className="mb-4 text-[15px] font-semibold leading-6 text-white sm:text-lg sm:leading-7">{currentQuestion.question_text}</p>
            ...
            {answerLoading && <p className="mt-2 text-[10px] text-slate-500">Saving answer…</p>}
            {currentFeedback && <div className={`mt-3 rounded-xl border p-3 text-xs ${currentFeedback.is_correct ? 'border-green-700 bg-green-950/30 text-green-200' : 'border-red-700 bg-red-950/30 text-red-200'}`}><p className="font-black">{currentFeedback.is_correct ? 'Correct ✓' : `Not quite. Correct answer: ${currentFeedback.correct_answer}`}</p>{currentFeedback.explanation && <p className="mt-1.5 leading-5 text-slate-300">{currentFeedback.explanation}</p>}</div>}
          </div>
        </main>

        <footer className="sticky bottom-0 z-20 mt-2 shrink-0 border-t border-slate-800 bg-slate-950/95 py-2 backdrop-blur">
          <div className="flex items-center justify-between gap-2"><button type="button" onClick={goPrevious} disabled={safeCurrentIndex === 0 && (mode !== 'cbt' || activeSectionKey === cbtSections[0]?.key)} className="rounded-xl border border-slate-700 px-3 py-2 text-xs font-bold text-slate-200 disabled:cursor-not-allowed disabled:opacity-30 sm:px-4">← Previous</button><div className="flex items-center gap-2"><button type="button" onClick={() => void handleSubmit(false)} disabled={submitting} className="rounded-xl bg-accent-500 px-3.5 py-2 text-xs font-black text-slate-950 shadow-lg shadow-accent-900/20 transition hover:bg-accent-400 disabled:opacity-60 sm:px-5">{submitting ? 'Submitting…' : 'Submit Exam'}</button>{safeCurrentIndex < sectionQuestions.length - 1 || (mode === 'cbt' && cbtSections.findIndex((section) => section.key === activeSectionKey) < cbtSections.length - 1) ? <button type="button" onClick={goNext} disabled={answerLoading || submitting} className="rounded-xl bg-brand-600 px-3.5 py-2 text-xs font-black text-white transition hover:bg-brand-500 disabled:opacity-60 sm:px-4">Next →</button> : null}</div></div>
          <p className="mt-1 text-center text-[9px] text-slate-600">You can submit with unanswered questions. They will be counted as unanswered, not as wrong.</p>
        </footer>
      </div>
    </div>
  );
