        ...
        <div className="fixed inset-0 z-[100] md:hidden">
          ...
          <aside className="absolute right-0 top-0 flex h-full w-[88%] max-w-sm flex-col overflow-hidden bg-white shadow-2xl">
            ...
            <nav className="flex-1 overflow-y-auto px-4 py-5">
                  ...
                  <div className="space-y-1">
                    {group.items.map((item) => (
                      <Link
                        ...
                        href={item.href}
                        onClick={closeMenu}
                        className="group flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-slate-700 transition hover:bg-emerald-50 hover:text-emerald-700"
                      >
                        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-50 text-lg transition group-hover:bg-white">
                          {item.icon}
                        </span>
              ...
              <div className="rounded-2xl bg-emerald-50 p-4">
                ...
                <Link
                  href="/community"
                  onClick={closeMenu}
                  className="mt-3 flex w-full items-center justify-center rounded-xl bg-emerald-600 px-3 py-3 text-xs font-black text-white transition hover:bg-emerald-700"
                >
                  Join Community →
                </Link>
              </div>
            ...
            <div className="border-t border-slate-100 p-4">
              <LogoutButton className="w-full rounded-xl px-3 py-3 text-left text-sm font-semibold text-slate-500 transition hover:bg-red-50 hover:text-red-600" />
            </div>
          </aside>
        </div>
      )}
